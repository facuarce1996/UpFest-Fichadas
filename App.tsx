        ...
        <div className="bg-white rounded-[32px] md:rounded-[40px] p-6 md:p-10 border border-slate-200 shadow-2xl shadow-slate-200/50">
          ...
          <div className="flex flex-col gap-4">
            ...
            <button 
              ...
              className="w-full h-14 md:h-16 rounded-xl md:rounded-2xl font-black text-xs uppercase tracking-widest text-slate-400 hover:bg-slate-50 transition border border-slate-100"
            >
  ...
  if (user.role === Role.ADMIN) {
    return (
      <div className="max-w-7xl mx-auto p-4 md:p-12 space-y-8 md:space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div className="bg-white rounded-[32px] md:rounded-[48px] p-6 md:p-10 border border-slate-200 shadow-sm overflow-hidden">
           <div className="flex flex-col md:flex-row items-center justify-between mb-8 md:mb-10 gap-6">
              <div className="text-center md:text-left">
                <h3 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tighter uppercase flex items-center justify-center md:justify-start gap-3">
                    ...
                    <tr key={log.id} className="hover:bg-white transition-colors group">
                      <td className="p-4">
                        <div 
                          ...
                          className="relative w-12 h-12 md:w-16 md:h-16 mx-auto rounded-xl md:rounded-2xl overflow-hidden cursor-zoom-in border-2 border-white shadow-sm hover:scale-105 transition-transform"
                        >
                          {log.photoEvidence ? (
                            <>
                          ...
                          ) : <UserIcon className="m-auto mt-2 md:mt-4 text-slate-300" />}
                        </div>
                      </td>
                      <td className="p-4 md:p-6">
                        <div className="flex flex-col">
                          <span className="font-black text-slate-900 uppercase tracking-tight text-sm md:text-base">{log.userName}</span>
                      ...
                      <td className="p-4 md:p-6 text-center text-xs font-black text-slate-900 tabular-nums">
        ...
        {zoomedImage && (
          <div 
            className="fixed inset-0 z-[200] bg-slate-900/90 backdrop-blur-md flex items-center justify-center p-4 md:p-8 animate-in fade-in duration-300"
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 md:p-12 space-y-8 md:space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="max-w-xl mx-auto w-full">
        {!deviceLocation ? (
          <div className="bg-white rounded-[32px] md:rounded-[40px] p-8 md:p-12 border border-orange-200 shadow-xl text-center space-y-6">
            <div className="w-16 h-16 md:w-20 md:h-20 bg-orange-100 text-orange-600 rounded-full flex items-center justify-center mx-auto">
              <Laptop size={32} />
            </div>
            <h3 className="text-xl md:text-2xl font-black text-slate-900 tracking-tighter uppercase">TERMINAL NO CONFIGURADA</h3>
            ...
            <button onClick={onLogout} className="w-full bg-slate-900 text-white py-4 md:py-5 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl">Cerrar Sesión</button>
          </div>
        ) : (
          <div className="bg-white rounded-[32px] md:rounded-[40px] p-6 md:p-10 border border-slate-200 shadow-xl shadow-slate-200/50">
            <div className="flex items-center justify-between mb-8 pb-8 border-b border-slate-100">
              <div className="flex items-center gap-4 md:gap-6">
                <div className="w-16 h-16 md:w-20 md:h-20 rounded-2xl md:rounded-3xl bg-slate-900 flex items-center justify-center text-white shadow-2xl shadow-slate-900/20 overflow-hidden border-2 md:border-4 border-white shrink-0">
                  {user.referenceImage ? <img src={user.referenceImage} className="w-full h-full object-cover" /> : <UserIcon size={32} />}
                </div>
                <div>
                  <h2 className="text-xl md:text-2xl font-black text-slate-900 tracking-tighter uppercase line-clamp-1">{user.name}</h2>
                  <div className="flex flex-wrap items-center gap-2 mt-1">
                    <span className="text-[8px] md:text-[10px] font-black bg-orange-100 text-orange-700 px-2 md:px-3 py-1 rounded-full uppercase tracking-widest">DNI: {user.dni}</span>
                    <span className="text-[8px] md:text-[10px] font-black bg-slate-100 text-slate-400 px-2 md:px-3 py-1 rounded-full uppercase tracking-widest">{user.role}</span>
                  </div>
                </div>
              </div>
              <div className="text-right hidden sm:block">
                <span className="block text-[8px] font-black text-slate-400 uppercase tracking-widest">Ubicación Terminal</span>
                <div className="flex items-center justify-end gap-1.5 text-orange-600 font-black text-xs uppercase">
                  <MapPinned size={14} />
                  {deviceLocation.name}
                </div>
              </div>
            ...
            <div className="space-y-6">
              ...
              <div className="relative aspect-square rounded-[24px] md:rounded-[32px] overflow-hidden bg-slate-900 shadow-inner group">
                {!cameraActive && !photo && (
              ...
              <div className="space-y-4">
                 {turnCompletedToday && (
                    <div className="bg-emerald-50 p-5 md:p-6 rounded-2xl border border-emerald-100 space-y-3">
              ...
              <div className="flex gap-4">
                <button 
                  ...
                  className="flex-1 bg-slate-900 text-white h-16 md:h-20 rounded-[20px] md:rounded-[28px] font-black text-xs uppercase tracking-[0.2em] shadow-2xl shadow-slate-900/20 disabled:opacity-20 flex flex-col items-center justify-center transition-all hover:translate-y-[-2px]"
                >
                ...
                <button 
                  ...
                  className="flex-1 bg-white border-2 border-slate-100 text-slate-900 h-16 md:h-20 rounded-[20px] md:rounded-[28px] font-black text-xs uppercase tracking-[0.2em] shadow-xl shadow-slate-200/50 disabled:opacity-20 flex flex-col items-center justify-center transition-all hover:translate-y-[-2px]"
                >
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 max-h-[80vh] overflow-y-auto pr-2 custom-scrollbar">
      ...
      <div className="flex gap-4 pt-6 border-t sticky bottom-0 bg-white z-10">
          <button type="button" onClick={onCancel} disabled={isSubmitting} className="flex-1 px-6 py-4 border border-slate-200 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-400 hover:bg-slate-50 transition">Cancelar</button>
          <button type="submit" disabled={isSubmitting} className="flex-1 bg-slate-900 text-white px-6 py-4 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-800 transition shadow-xl shadow-slate-200 disabled:opacity-50">
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div>
            <h1 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tighter">PERSONAL</h1>
            <p className="text-slate-400 font-bold text-[10px] uppercase tracking-widest mt-1">Gestión de RRHH UpFest</p>
          </div>
          <button onClick={() => setIsCreating(true)} className="w-full md:w-auto bg-slate-900 text-white px-6 py-3 rounded-2xl flex items-center justify-center gap-2 hover:bg-slate-800 transition shadow-xl shadow-slate-200 font-black text-[10px] uppercase tracking-widest"><Plus size={18} /> Nuevo Usuario</button>
      </div>
      
      <div className="bg-white rounded-3xl md:rounded-[32px] border border-slate-200 overflow-hidden shadow-sm">
         <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse min-w-[600px]">
               <thead>
                   <tr className="bg-slate-50 border-b border-slate-200">
                       <th className="p-4 md:p-6 text-[10px] font-black text-slate-900 uppercase tracking-widest">Personal</th>
                       <th className="p-4 md:p-6 text-[10px] font-black text-slate-900 uppercase tracking-widest">DNI</th>
                       <th className="p-4 md:p-6 text-[10px] font-black text-slate-900 uppercase tracking-widest text-center">Rol</th>
                       <th className="p-4 md:p-6 text-[10px] font-black text-slate-900 uppercase tracking-widest text-right">Acciones</th>
                   </tr>
               </thead>
               <tbody className="divide-y divide-slate-100">
                   {loading ? (
                       ...
                       <tr key={u.id} className="hover:bg-slate-50 transition-colors">
                           <td className="p-4 md:p-6">
                               <div className="flex items-center gap-4">
                                   <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-slate-100 overflow-hidden border border-slate-200 shrink-0 shadow-sm">
                                       {u.referenceImage ? <img src={u.referenceImage} className="w-full h-full object-cover" /> : <UserIcon size={18} className="m-auto mt-2.5 md:mt-3 text-slate-300"/>}
                                   </div>
                                   <div>
                                       <span className="font-black text-slate-800 block tracking-tight text-sm md:text-base">{u.name}</span>
                                       <span className="text-[9px] text-slate-400 font-black uppercase tracking-wider">{u.role}</span>
                                   </div>
                               </div>
                           </td>
                           <td className="p-4 md:p-6 text-xs md:text-sm font-mono font-bold text-slate-900 tracking-tighter">{u.dni}</td>
                           <td className="p-4 md:p-6 text-center text-[9px] md:text-[10px] font-black text-slate-900 uppercase tracking-tighter">{u.role}</td>
                           <td className="p-4 md:p-6 text-right">
                               <div className="flex justify-end gap-1 md:gap-2">
                                   <button onClick={() => setEditingUser(u)} className="p-2 md:p-3 text-slate-400 hover:text-orange-600 hover:bg-orange-50 rounded-xl transition shadow-sm border border-transparent hover:border-orange-100" title="Editar"><Pencil size={16}/></button>
                                   <button onClick={() => handleDelete(u.id)} className="p-2 md:p-3 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition shadow-sm border border-transparent hover:border-red-100" title="Eliminar"><Trash2 size={16}/></button>
                               </div>
                           </td>
                       </tr>
                   ))}
      ...
      {(isCreating || editingUser) && (
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-2 md:p-4">
              <div className="bg-white rounded-[32px] md:rounded-[40px] p-6 md:p-10 w-full max-w-lg shadow-2xl animate-in fade-in zoom-in-95 duration-200">
                  <div className="flex justify-between items-center mb-6 md:mb-8">
                      <div>
                        <h3 className="font-black text-xl md:text-2xl text-slate-900 tracking-tighter">{isCreating ? 'NUEVO USUARIO' : 'EDITAR USUARIO'}</h3>
...
const LocationsDashboard = () => {
    ...
    return (
        <div className="max-w-7xl mx-auto p-4 md:p-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                <div>
                  <h1 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tighter">SALONES</h1>
                  <p className="text-slate-400 font-bold text-[10px] uppercase tracking-widest mt-1">Sedes UpFest & Geocercas</p>
                </div>
                <button onClick={() => setIsCreating(true)} className="w-full md:w-auto bg-slate-900 text-white px-6 py-3 rounded-2xl flex items-center justify-center gap-2 hover:bg-slate-800 transition shadow-xl shadow-slate-200 font-black text-[10px] uppercase tracking-widest"><Plus size={18} /> Nuevo Salón</button>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
                {loading ? (
                    ...
                    <div key={loc.id} className={`bg-white rounded-[28px] md:rounded-[32px] p-6 md:p-8 border ${currentDeviceLocId === loc.id ? 'border-orange-500 ring-4 ring-orange-500/10' : 'border-slate-200'} shadow-sm hover:shadow-xl transition-all group relative overflow-hidden flex flex-col`}>
                        ...
                        <div className="flex flex-wrap items-center gap-2 text-[8px] md:text-[10px] font-black text-slate-300 mb-6 md:mb-8">
                            <span className="bg-slate-50 px-2 md:px-3 py-1.5 rounded-full tracking-tighter border border-slate-100 text-slate-600">LAT: {loc.lat.toFixed(4)}</span>
                            <span className="bg-slate-50 px-2 md:px-3 py-1.5 rounded-full tracking-tighter border border-slate-100 text-slate-600">LNG: {loc.lng.toFixed(4)}</span>
                            <span className="bg-orange-600 text-white px-2 md:px-3 py-1.5 rounded-full tracking-tighter border border-orange-700">R: {loc.radiusMeters}M</span>
                        </div>

                        <div className="mt-auto pt-6 border-t border-slate-50 flex gap-2 md:gap-3">
                           {currentDeviceLocId !== loc.id && (
                              <button 
                                onClick={() => handleSetTerminalLocation(loc.id)}
                                className="flex-1 bg-slate-900 text-white px-3 md:px-4 py-2.5 md:py-3 rounded-xl text-[9px] md:text-[10px] font-black uppercase tracking-widest hover:bg-slate-800 transition shadow-lg"
                              >
                                Vincular
                              </button>
                           )}
                           <button onClick={() => setEditingLoc(loc)} className="p-2.5 md:p-3 bg-slate-100 text-slate-500 hover:text-orange-600 rounded-xl transition-transform hover:scale-105" title="Editar"><Pencil size={16}/></button>
                           <button onClick={() => handleDelete(loc.id)} className="p-2.5 md:p-3 bg-slate-100 text-slate-500 hover:text-red-600 rounded-xl transition-transform hover:scale-105" title="Eliminar"><Trash2 size={16}/></button>
                        </div>
                    </div>
                ))}
            </div>
            ...
            {(isCreating || editingLoc) && (
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-2 md:p-4">
                    <div className="bg-white rounded-[32px] md:rounded-[40px] p-6 md:p-10 w-full max-w-md shadow-2xl animate-in fade-in zoom-in-95 duration-200">
                        <div className="flex justify-between items-center mb-6 md:mb-8">
                            <div>
                              <h3 className="font-black text-xl md:text-2xl text-slate-900 tracking-tighter">{isCreating ? 'AÑADIR SEDE' : 'EDITAR SEDE'}</h3>
...
const PayrollDashboard = () => {
    ...
    return (
        <div className="p-4 md:p-6 max-w-[1600px] mx-auto">
            ...
            <div className="bg-white rounded-[28px] md:rounded-[40px] shadow-sm border border-slate-200 p-6 md:p-10 mb-8 flex flex-col md:flex-row items-end gap-4 md:gap-6">
                <div className="grid grid-cols-2 gap-4 md:gap-6 flex-1 w-full">
                    <div className="space-y-2">
                        ...
                        <input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="w-full px-4 md:px-6 py-3 md:py-4 bg-white border border-slate-200 rounded-xl md:rounded-2xl focus:ring-8 focus:ring-orange-500/5 outline-none transition font-extrabold text-sm md:text-base" />
                    </div>
                    <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-900 uppercase tracking-widest">Fecha Fin</label>
                        <input type="date" value={endDate} onChange={e => setEndDate(e.target.value)} className="w-full px-4 md:px-6 py-3 md:py-4 bg-white border border-slate-200 rounded-xl md:rounded-2xl focus:ring-8 focus:ring-orange-500/5 outline-none transition font-extrabold text-sm md:text-base" />
                    </div>
                </div>
...
const Sidebar = ({ activeTab, setActiveTab, currentUser, onLogout, logoUrl, isMobileMenuOpen, setIsMobileMenuOpen }: { activeTab: string, setActiveTab: (t: string) => void, currentUser: User, onLogout: () => void, logoUrl: string | null, isMobileMenuOpen: boolean, setIsMobileMenuOpen: (v: boolean) => void }) => {
  const NavButton = ({ tab, icon: Icon, label }: { tab: string, icon: any, label: string }) => (
    <button 
      onClick={() => { setActiveTab(tab); setIsMobileMenuOpen(false); }} 
      className={`w-full flex items-center gap-4 px-6 py-4 rounded-[20px] text-[10px] font-black uppercase tracking-widest transition ${activeTab === tab ? 'bg-orange-50 text-orange-700 shadow-sm' : 'text-slate-400 hover:bg-slate-50'}`}
    >
      <Icon size={20}/> {label}
    </button>
  );
  ...
  return (
    <>
      ...
      <aside className={`
        fixed inset-y-0 left-0 z-[100] w-72 bg-white border-r border-slate-200 shadow-xl transform transition-transform duration-300 md:translate-x-0 md:static md:shadow-sm h-screen flex flex-col
      ...
      `}>
          ...
          <nav className="flex-1 p-8 space-y-2 overflow-y-auto">
              <NavButton tab="clock" icon={Clock} label={currentUser.role === Role.ADMIN ? 'Monitor' : 'Fichadas'} />
              {currentUser.role === Role.ADMIN && (
                  <>
                      <NavButton tab="payroll" icon={Wallet} label="Liquidaciones" />
                      <NavButton tab="admin" icon={Users} label="Personal" />
          ...
          <div className="p-8 border-t border-slate-50">
              <div className="mb-4 px-6 py-2 bg-slate-50 rounded-2xl">
                ...
                <span className="text-[10px] font-black text-slate-600 truncate block">{currentUser.dni}</span>
              </div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex font-sans flex-col md:flex-row overflow-hidden">
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
          {/* Header para móvil */}
          <header className="md:hidden bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between z-40 shadow-sm shrink-0">
             <button onClick={() => setIsMobileMenuOpen(true)} className="p-2 -ml-2 text-slate-900"><Menu size={24}/></button>
             <div className="flex items-center gap-2">
                <span className="font-black text-slate-900 tracking-tighter text-lg">UPFEST</span>
             </div>
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-4 md:p-6 bg-slate-50 font-sans">
      <div className="w-full max-sm:px-4 max-w-sm bg-white rounded-[40px] md:rounded-[48px] shadow-2xl p-10 md:p-14 space-y-10 md:space-y-12 border border-slate-100 relative overflow-hidden">
        ...
        <div className="text-center">
          ...
          <h2 className="text-3xl md:text-4xl font-black text-slate-900 tracking-tighter mb-2 uppercase">ENTRAR</h2>
          <p className="text-slate-400 font-black text-[10px] uppercase tracking-[0.3em]">Acceso con DNI / Legajo</p>
        </div>
        <form onSubmit={handleLogin} className="space-y-6">
          <div className="space-y-2">
            <label className="text-[10px] font-black text-slate-900 uppercase tracking-widest ml-4">Tu Identificador</label>
            <input type="text" value={dni} onChange={e => setDni(e.target.value)} className="w-full px-6 md:px-8 py-5 md:py-6 bg-white border border-slate-200 rounded-[20px] md:rounded-[24px] focus:ring-8 focus:ring-orange-500/5 outline-none font-extrabold placeholder:text-slate-300 text-base md:text-lg shadow-inner text-slate-900" placeholder="00.000.000" required />
          </div>
          <div className="space-y-2">
            <label className="text-[10px] font-black text-slate-900 uppercase tracking-widest ml-4">Contraseña</label>
            <input type="password" value={password} onChange={e => setPassword(e.target.value)} className="w-full px-6 md:px-8 py-5 md:py-6 bg-white border border-slate-200 rounded-[20px] md:rounded-[24px] focus:ring-8 focus:ring-orange-500/5 outline-none font-extrabold placeholder:text-slate-300 text-base md:text-lg shadow-inner text-slate-900" placeholder="••••••••" required />
          </div>
          {error && <div className="text-red-500 text-[10px] text-center font-black bg-red-50 p-4 rounded-[20px] border border-red-100 tracking-[0.2em] uppercase">{error}</div>}
          <button type="submit" disabled={loading} className={`w-full bg-slate-900 hover:bg-slate-800 text-white font-black py-5 md:py-6 rounded-[20px] md:rounded-[24px] transition-all transform hover:scale-[1.02] active:scale-[0.98] shadow-2xl shadow-slate-200 uppercase tracking-widest text-xs ${loading ? 'opacity-50' : ''}`}>
